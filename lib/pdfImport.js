...
const NOISE = ['بيان', 'أسعار', 'اسعار', 'الإجمالي', 'الاجمالي', 'الصنف', 'المجموع', 'صفحة', 'التاريخ',
  'رقم', 'الرقم', 'الكود', 'السعر', 'الكمية', 'ملاحظات', 'العميل', 'الاسم', 'الهاتف', 'التليفون',
  'العنوان', 'المندوب', 'الشركة', 'السقا', 'اجمالي', 'م'];
...
async function readRows(file, opts = {}) {
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    for (const item of content.items) {
      ...
      byY.get(y).push({ x: item.transform[4], text: normDigits(item.str.trim()) });
    }
    for (const [, items] of [...byY.entries()].sort((a, b) => b[0] - a[0])) {
      items.sort((a, b) => b.x - a.x); // يمين → شمال (عربي)
      rows.push(items);
    }
  }
  if (!textItems) {
...
export function detectNumberColumns(allTokens) {
  ...
  const priceCols = strong.slice(0, strong.length - 1);   // الباقي شمال = أسعار
...
export function classifyByColumns(toks, cols) {
  ...
  const code = codeTok ? codeTok.text : '';
  const prices = [];
  for (const col of cols.priceCols) {
    const pt = toks.filter((t) => isNum(t.text) && inCol(t.x, col)).sort((a, b) => a.x - b.x)[0];
    if (pt) prices.push(toNum(pt.text));
  }
  const pos = prices.filter((n) => n > 0);
  let price = 0, cost = 0;
  if (pos.length >= 2) { price = Math.max(...pos); cost = Math.min(...pos); }
  else if (pos.length === 1) price = pos[0];

  // اسم الصنف = كل الكلام اللي مش في عمود الكود ولا الأسعار (بيشمل أرقام الاسم)
  const priceMaxX = cols.priceCols.length ? Math.max(...cols.priceCols.map((c) => c.max + 12)) : -Infinity;
  const nameToks = toks
    .filter((t) => {
      if (isNum(t.text) && inCol(t.x, cols.codeCol)) return false;
      if (isNum(t.text) && cols.priceCols.some((c) => inCol(t.x, c))) return false;
      if (t.x <= priceMaxX && isNum(t.text)) return false;
      return !NOISE.includes(t.text);
    })
    .sort((a, b) => b.x - a.x);
  // المورد = كلمة عربية في يمين الكود
  let supplier = '';
  if (codeTok) {
    const supTok = nameToks.find((t) => hasArabic(t.text) && t.x > codeTok.x);
    if (supTok) supplier = supTok.text;
  }
  let nameArr = nameToks.filter((t) => t.text !== supplier || t.x <= (codeTok ? codeTok.x : Infinity));
  if (supplier) nameArr = nameArr.filter((t) => !(t.text === supplier && t.x > (codeTok ? codeTok.x : 0)));
  const name = cleanName(nameArr.map((t) => t.text).join(' '));
  if (!name || name.length < 2) return null;
  if (!code && !price) return null;
