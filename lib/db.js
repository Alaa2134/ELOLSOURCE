...
function saveLocal(key, value) {
  ...
  try {
    ...
    // تخزين الجهاز اتملى (غالباً من صور الأصناف المتخزنة كـ base64).
    // نجرب تاني من غير الصور بدل ما البيانات كلها تضيع.
    try {
      if (Array.isArray(value) && value.some((x) => x && x.image)) {
        localStorage.setItem(key, JSON.stringify(value.map((x) => (x && x.image ? { ...x, image: '' } : x))));
        return true;
      }
    } catch {}
    console.warn('تخزين الجهاز اتملى — ' + key, e);
    return false;
  }
}
...
export async function flushPending() {
  ...
  try {
    ...
    while (pending.length) {
      ...
      if (error) {
        // جدول لسه متعملش في السحابة؟ نعدّي العملية دي بدل ما توقف مزامنة كل حاجة
        // (البيانات محفوظة محلياً، وهترفع أول ما الجدول يتعمل)
        if (isMissingTable(error)) {
          pending.shift();
          saveLocal(KEYS.pending, pending);
          continue;
        }
        break; // خطأ مؤقت (نت مثلاً) — نحاول تاني في المرة الجاية
      }
      pending.shift();
      saveLocal(KEYS.pending, pending);
    }
...
export async function pushAllToCloud() {
  ...
  try {
    ...
    for (const [table, list] of Object.entries(tables)) {
      ...
      let skipTable = false;
      for (let i = 0; i < rows.length && !skipTable; i += 400) {
        const { error } = await client.from(table).upsert(rows.slice(i, i + 400));
        if (error) {
          // جدول ناقص؟ نعدّيه ونكمّل باقي الجداول بدل ما الرفعة كلها تفشل
          if (isMissingTable(error)) { skipTable = true; break; }
...
export async function syncPull() {
  ...
  try {
    for (const table of ['products', 'customers', 'invoices', 'payments', 'expenses', 'suppliers', 'purchases', 'quotes', 'stocktakes', 'daycloses', 'audit']) {
...
export async function resetFromCloud() {
  ...
  const tables = ['products', 'customers', 'invoices', 'payments', 'expenses', 'suppliers', 'purchases', 'quotes', 'stocktakes', 'daycloses', 'audit'];
  try {
...
export const DEFAULT_SETTINGS = {
  ...
  alerts: {
    ...
    minMargin: 5, // % — أقل نسبة ربح مقبولة على الفاتورة، أقل من كده تحذير برتقالي
  },
  // مندوبين تطبيق الأسعار: كل واحد له كود دخول خاص [{ name, code }]
  repUsers: [],
  ...
  // متجر الجملة للتجار — بيعرض الأصناف بسعر البيع
  store: {
    showOnInvoice: true, // نطبع QR ولينك المتجر على الفاتورة
    minOrder: 0, // أقل قيمة للطلب من المتجر (0 = من غير حد)
  },
  wa: {
    gatewayUrl: '',
    token: '',
...
export function getSettings() {
  ...
  const merged = {
    ...
    repUsers: Array.isArray(s.repUsers) ? s.repUsers : [],
  };
...
// تعبئة "سعر النقدي" بالجملة = سعر البيع + نسبة% — بدل ما تملاه صنف صنف
// mode: 'empty' يطبّق على اللي سعره النقدي فاضي بس · 'all' على الكل
export function bulkSetRetailPrice(pct, mode = 'empty') {
  const list = loadLocal(KEYS.products, []);
  const factor = 1 + (Number(pct) || 0) / 100;
  const now = todayISO();
  let changed = 0;
  for (const p of list) {
    const base = Number(p.price) || 0;
    if (base <= 0) continue; // مفيش سعر بيع نبني عليه
    if (mode === 'empty' && Number(p.priceRetail) > 0) continue; // مش بنلمس المملوء
    const rp = Math.round(base * factor * 100) / 100;
    if (rp !== Number(p.priceRetail)) {
      p.priceRetail = rp;
      p.updated_at = now;
  ...
  if (changed) {
    saveLocal(KEYS.products, list);
    logAudit('تعبئة سعر النقدي', `تعبئة سعر النقدي لـ ${changed} صنف (سعر البيع + ${pct}%)`);
  }
  return changed;
}
...
export function listQuotes() {
  return loadLocal(KEYS.quotes, []).sort((a, b) => (b.number || 0) - (a.number || 0));
}
export function getQuote(id) {
  return loadLocal(KEYS.quotes, []).find((q) => q.id === id) || null;
}
export function nextQuoteNumber() {
  return listQuotes().reduce((m, q) => Math.max(m, Number(q.number) || 0), 0) + 1;
}
export function saveQuote(q) {
  const list = loadLocal(KEYS.quotes, []);
  const row = { ...q, id: q.id || uid(), updated_at: todayISO() };
  const i = list.findIndex((x) => x.id === row.id);
  if (i >= 0) list[i] = row; else list.push(row);
  saveLocal(KEYS.quotes, list);
  queuePush('quotes', 'upsert', row);
  logAudit('عرض سعر', `عرض سعر ${row.number} لـ ${row.customer?.name || ''} — ${row.items?.length || 0} صنف`);
  return row;
}
export function setQuoteStatus(id, status) {
  ...
  list[i] = { ...list[i], status, updated_at: todayISO() };
  saveLocal(KEYS.quotes, list);
  queuePush('quotes', 'upsert', list[i]);
  return list[i];
}
export function deleteQuote(id) {
  saveLocal(KEYS.quotes, loadLocal(KEYS.quotes, []).filter((q) => q.id !== id));
  queuePush('quotes', 'delete', { id });
}
...
export async function submitStoreOrder(order) {
  ...
  if (error) {
    // رسالة مفهومة للتاجر بدل رسالة السيرفر الإنجليزية
    if (isMissingTable(error)) throw new Error('استقبال الطلبات لسه مش متفعّل عند المحل — ابعت طلبك واتساب من الزر اللي تحت 👇');
    throw new Error('حصلت مشكلة في الإرسال — ابعت طلبك واتساب من الزر اللي تحت 👇');
  }
  return row;
}

// هل الخطأ سببه إن الجدول لسه متعملش في السحابة؟ (لازم الأدمن يشغّل كود الجداول)
export function isMissingTable(error) {
  const m = String(error?.message || '') + String(error?.code || '');
  return /schema cache|does not exist|42P01|Could not find the table/i.test(m);
}

// بيرجّع true لو جدول معيّن لسه متعملش في السحابة — عشان نوضّح للأدمن بدل الصمت
export async function cloudTableMissing(table) {
  const client = getSupabase();
  if (!client) return false;
  try {
...
// فحص شامل: أنهي جداول ناقصة في السحابة — بيتعرض في لوحة الأدمن
// عشان المستخدم يعرف بالظبط إيه اللي مش بيتزامن بدل ما يخمّن
export const CLOUD_TABLES = [
  { name: 'products', label: 'الأصناف' },
  { name: 'customers', label: 'العملاء' },
  { name: 'invoices', label: 'الفواتير' },
  { name: 'payments', label: 'سندات القبض' },
  { name: 'expenses', label: 'المصاريف' },
  { name: 'suppliers', label: 'الموردين' },
  { name: 'purchases', label: 'المشتريات والطلبات' },
  { name: 'stocktakes', label: 'الجرد' },
  { name: 'daycloses', label: 'إقفال اليومية' },
  { name: 'audit', label: 'سجل العمليات' },
  { name: 'settings', label: 'الإعدادات' },
  { name: 'store_orders', label: 'طلبات الفواتير والتحصيل' },
  { name: 'quotes', label: 'عروض الأسعار' },
];

export async function checkCloudTables() {
  const client = getSupabase();
  if (!client) return null;
  // بالتوازي مع مهلة لكل جدول — عشان جدول بطيء مايعلّقش الفحص كله
  return Promise.all(
    CLOUD_TABLES.map(async (t) => {
      try {
        const { error } = await withTimeout(client.from(t.name).select('id').limit(1), 12000, t.name);
        return { ...t, ok: !(error && isMissingTable(error)) };
      } catch {
        // مهلة = مش عارفين. مانقولش "موجود" ولا "ناقص" — الاتنين ممكن يضللوا
        return { ...t, ok: null };
      }
    })
  );
...
// ---------- تاريخ حركة الصنف ----------
// كل حركة على صنف واحد: مين اشتراه وإمتى وبكام، ومنين اشتريناه وبكام
// بيرجع { product, sales, buys, summary } — مرتّبين من الأحدث للأقدم
export function itemHistory(code) {
  const key = String(code || '');
  if (!key) return { product: null, sales: [], buys: [], summary: null };
  const product = listProducts().find((p) => String(p.code) === key) || null;

  const sales = [];
  for (const inv of loadLocal(KEYS.invoices, [])) {
    for (const it of inv.items || []) {
      if (String(it.code) !== key) continue;
      const qty = Number(it.stockQty ?? it.qty) || 0;
      ...
      sales.push({
        ...
        total,
        // سعر القطعة الفعلي بعد الخصم — ده اللي بيتقارن بالتكلفة
        unitNet: qty > 0 ? Math.round((total / qty) * 100) / 100 : 0,
      });
    }
  }

  const buys = [];
  for (const pu of loadLocal(KEYS.purchases, [])) {
    if (pu.kind === 'طلب') continue; // الطلبات لسه مجتش، مش حركة مخزون
    for (const it of pu.items || []) {
      ...
      buys.push({
        ...
        date: pu.date,
        number: pu.number,
        supplier: pu.supplier?.name || 'مورد',
        qty: Number(it.qty) || 0,
        cost: Number(it.cost) || 0,
      });
    }
  ...
  const byDate = (a, b) => String(b.date || '').localeCompare(String(a.date || '')) || (b.number || 0) - (a.number || 0);
  sales.sort(byDate);
  buys.sort(byDate);

  const soldQty = sales.reduce((s, x) => s + x.qty, 0);
  const soldValue = sales.reduce((s, x) => s + x.total, 0);
  const boughtQty = buys.reduce((s, x) => s + x.qty, 0);
  const boughtValue = buys.reduce((s, x) => s + x.qty * x.cost, 0);
  ...
  return {
    ...
    summary: {
      soldQty,
      soldValue: rnd(soldValue),
      avgSell: soldQty > 0 ? rnd(soldValue / soldQty) : 0,
      minSell: sales.length ? Math.min(...sales.map((x) => x.unitNet)) : 0,
      maxSell: sales.length ? Math.max(...sales.map((x) => x.unitNet)) : 0,
      lastSale: sales[0]?.date || '',
      boughtQty,
      avgBuy: boughtQty > 0 ? rnd(boughtValue / boughtQty) : 0,
      lastBuy: buys[0]?.date || '',
      lastSupplier: buys[0]?.supplier || '',
      cost,
      // الربح المحقق من كل اللي اتباع بالتكلفة الحالية (تقريبي — التكلفة متوسط متحرّك)
      profit: cost > 0 ? rnd(soldValue - soldQty * cost) : null,
      stock: Number(product?.stock) || 0,
      // كام عميل مختلف اشتروه — بيقولك الصنف ده دارج ولا زبون واحد بس بياخده
      customers: new Set(sales.map((x) => x.customer)).size,
    },
  };
}
...
// ---------- طلبات المراجعة ----------
// المحاسب أو الأدمن بيعلّم على أي مستند فيه شك. العلامة بتتحط على المستند نفسه
// (مش جدول جديد) فبتتزامن مع باقي البيانات على طول.
// مهم: العلامة مابتوقفش أي شغل ولا بتلغي الحركة — دي تنبيه للمراجعة بس.
const REVIEW_STORES = {
  invoice: 'invoices',
  return: 'invoices',   // المرتجع فاتورة من نوع "مرتجع"
  payment: 'payments',
  expense: 'expenses',
};

export function flagForReview(docType, id, { reason = '', by = '' } = {}) {
  const key = REVIEW_STORES[docType];
  if (!key) return null;
  const list = loadLocal(KEYS[key], []);
  const i = list.findIndex((x) => x.id === id);
  if (i < 0) return null;
  list[i] = {
    ...list[i],
    review: { open: true, reason: String(reason || '').trim(), by, at: new Date().toISOString() },
    updated_at: todayISO(),
  };
  saveLocal(KEYS[key], list);
  queuePush(key, 'upsert', list[i]);
  logAudit('طلب مراجعة', `${docType} ${list[i].number || id}${reason ? ' — ' + reason : ''}`);
  reviewChanged();
  return list[i];
}

export function resolveReview(docType, id, { note = '', by = '' } = {}) {
  const key = REVIEW_STORES[docType];
  if (!key) return null;
  const list = loadLocal(KEYS[key], []);
  const i = list.findIndex((x) => x.id === id);
  if (i < 0 || !list[i].review) return null;
  list[i] = {
    ...list[i],
    review: { ...list[i].review, open: false, note: String(note || '').trim(), resolvedBy: by, resolvedAt: new Date().toISOString() },
    updated_at: todayISO(),
  };
  saveLocal(KEYS[key], list);
  queuePush(key, 'upsert', list[i]);
  logAudit('إقفال مراجعة', `${docType} ${list[i].number || id}${note ? ' — ' + note : ''}`);
  reviewChanged();
  return list[i];
}

// كل المستندات المعلّم عليها — المفتوحة الأول
export function listReviewFlags({ openOnly = true } = {}) {
  const out = [];
  const push = (docType, x, label, amount) => {
    if (!x.review) return;
    if (openOnly && !x.review.open) return;
    out.push({ docType, id: x.id, number: x.number, date: x.date, label, amount, review: x.review });
  };
  for (const inv of loadLocal(KEYS.invoices, [])) {
    const isRet = inv.type === 'مرتجع';
    push(isRet ? 'return' : 'invoice', inv,
      `${isRet ? 'مرتجع' : 'فاتورة'} ${inv.number} — ${inv.customer?.name || 'عميل نقدي'}`,
      Number(inv.totals?.net) || 0);
  }
  for (const p of loadLocal(KEYS.payments, [])) {
    push('payment', p, `سند قبض ${p.number} — ${p.customerName || ''}`, Number(p.amount) || 0);
  }
  for (const e of loadLocal(KEYS.expenses, [])) {
    push('expense', e, `مصروف — ${e.desc || ''}`, Number(e.amount) || 0);
  }
  return out.sort((a, b) => String(b.review.at || '').localeCompare(String(a.review.at || '')));
}

// الشِل بيسمع الحدث ده عشان البادج يتحدّث على طول من غير ما تتنقل لصفحة تانية
function reviewChanged() {
  if (typeof window !== 'undefined') window.dispatchEvent(new Event('saqqa-review-changed'));
}

export function openReviewCount() {
  try { return listReviewFlags({ openOnly: true }).length; } catch { return 0; }
}

// تحصيل المندوب عايش في السحابة — علامته بتتحط عليه هناك
export async function flagStoreOrderReview(id, { reason = '', by = '' } = {}) {
  const client = getSupabase();
  if (!client) return;
  await ensureSession(client);
  const { data } = await client.from('store_orders').select('data').eq('id', id).single();
...
// ---------- بيانات المندوب على موبايله ----------
// موبايل المندوب مابيعملش syncPull الكامل (صفحته bare)، فبنجيب اللي محتاجه بس:
// العملاء عشان يختار منهم، وفواتير عميل واحد لما يطلب كشف حسابه.
export async function fetchCustomersCloud() {
  const client = getSupabase();
  if (!client) return null;
  try {
    const all = [];
    for (let from = 0; ; from += 1000) {
      const { data, error } = await client.from('customers').select('id,data').range(from, from + 999);
      if (error) break;
      all.push(...(data || []));
      if (!data || data.length < 1000) break;
    }
...
export function saveCustomersLocal(list) {
  if (!Array.isArray(list) || !list.length) return false;
  return saveLocal(KEYS.customers, list);
}

// كشف حساب عميل من السحابة — فواتيره وسنداته والرصيد
// (الفلترة بتتم في PostgREST على الحقل جوه الـ jsonb)
export async function fetchCustomerAccountCloud(name) {
  const client = getSupabase();
  if (!client || !name) return null;
  try {
    const [inv, pay] = await Promise.all([
      client.from('invoices').select('id,data').eq('data->customer->>name', name).limit(300),
      client.from('payments').select('id,data').eq('data->>customerName', name).limit(300),
    ]);
    if (inv.error) return null;
    const invoices = (inv.data || []).map((r) => ({ ...r.data, id: r.id }));
    const payments = (pay.error ? [] : (pay.data || [])).map((r) => ({ ...r.data, id: r.id }));
    return buildAccount(name, invoices, payments);
  } catch {
    return null;
  }
}

// نفس الحساب بس من بيانات الجهاز — بيتستخدم لما مفيش نت
export function customerAccountLocal(name) {
  return buildAccount(name, loadLocal(KEYS.invoices, []).filter((i) => i.customer?.name === name),
    loadLocal(KEYS.payments, []).filter((p) => p.customerName === name));
}

function buildAccount(name, invoices, payments) {
  const rows = invoices
    ...
    .map((i) => ({
      ...
      number: i.number,
      date: i.date,
      net: Number(i.totals?.net) || 0,
      paid: Number(i.totals?.paid) || 0,
      remaining: Number(i.totals?.remaining) || 0,
      items: i.items?.length || 0,
    }))
    .sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
  const debt = rows.reduce((n, r) => n + Math.max(0, r.remaining), 0);
  const totalBought = rows.reduce((n, r) => n + r.net, 0);
  ...
  return {
    ...
    invoices: rows,
    lastPayment: lastPay ? { date: lastPay.date, amount: Number(lastPay.amount) || 0, number: lastPay.number } : null,
  };
}

// ---------- طلبات المندوب: طابور محلي عشان الطلب مايضيعش ----------
// المندوب بياخد الطلب وهو واقف عند التاجر — ممكن النت يكون قاطع خالص.
// الطلب بيتخزن على الجهاز وبيتبعت لوحده أول ما النت يرجع.
const REP_QUEUE_KEY = 'saqqa_rep_pending';

export function listPendingRepOrders() {
  return loadLocal(REP_QUEUE_KEY, []);
}

export function queueRepOrder(order) {
  const row = { ...order, id: order.id || uid(), queuedAt: new Date().toISOString() };
  const q = listPendingRepOrders();
  q.push(row);
  saveLocal(REP_QUEUE_KEY, q);
  return row;
}

function dropRepOrder(id) {
  saveLocal(REP_QUEUE_KEY, listPendingRepOrders().filter((o) => o.id !== id));
}

// بيحاول يبعت كل الطلبات المستنية — بيرجع عدد اللي اتبعت
// أي طلب بيفشل بيفضل في الطابور للمحاولة الجاية (مش بنمسحه)
export async function flushRepOrders() {
  if (typeof navigator !== 'undefined' && navigator.onLine === false) return 0;
  let sent = 0;
  for (const o of listPendingRepOrders()) {
    try {
      await submitStoreOrder(o);
      dropRepOrder(o.id);
      sent++;
    } catch {
      break; // النت أو الجدول لسه مش تمام — نسيب الباقي للمرة الجاية
    }
  }
  return sent;
}

// تحصيل المندوب: بيتبعت بنفس مسار الطلب (kind: 'تحصيل') عشان مانحتاجش جدول
// جديد، وعشان الكاشير هو اللي يعتمده ويعمل سند القبض — المندوب مايكتبش في الدفاتر
export async function sendRepCollection({ customer, amount, notes, rep }) {
  return sendRepOrder({
    kind: 'تحصيل',
    source: 'مندوب',
    rep: rep || '',
    trader: { name: customer?.name || '', phone: customer?.phone || '' },
    notes: notes || '',
    items: [],
    total: Math.round((Number(amount) || 0) * 100) / 100,
  });
}

// بيبعت الطلب على طول، ولو فشل بيحطه في الطابور بدل ما يضيع
// بيرجع { sent: true } أو { sent: false, queued: true, reason }
export async function sendRepOrder(order) {
  try {
    const saved = await submitStoreOrder(order);
    return { sent: true, order: saved };
  } catch (e) {
    const row = queueRepOrder(order);
    return { sent: false, queued: true, order: row, reason: e?.message || '' };
  }
}

// ---------- أوفلاين: حفظ الأسعار على الجهاز ----------
// صفحة الاستعلام بتجيب أحدث أسعار من السحابة — لازم تتخزن عشان تشتغل من غير نت
const PRICES_SYNC_KEY = 'saqqa_prices_synced_at';

export function saveProductsLocal(list) {
  if (!Array.isArray(list) || !list.length) return false;
  const ok = saveLocal(KEYS.products, dedupByCode(list));
  if (ok) { try { localStorage.setItem(PRICES_SYNC_KEY, new Date().toISOString()); } catch {} }
  return ok;
}

// آخر مرة اتحدّثت فيها الأسعار من السحابة على الجهاز ده
export function pricesSyncedAt() {
  if (typeof window === 'undefined') return null;
  return localStorage.getItem(PRICES_SYNC_KEY);
}

// ---------- نسخ احتياطي ----------
// الجداول اللي بتترحّل في النسخة الاحتياطية — أي جدول جديد يتضاف هنا وبس
const BACKUP_TABLES = [
  'products', 'customers', 'invoices', 'payments', 'expenses',
  'suppliers', 'purchases', 'quotes', 'stocktakes', 'daycloses', 'audit',
];

export function exportBackup() {
  const out = { exported_at: new Date().toISOString(), app: 'saqqa', version: 2 };
  for (const t of BACKUP_TABLES) out[t] = loadLocal(KEYS[t], []);
...
export function importBackup(json) {
  ...
  if (!data || typeof data !== 'object') throw new Error('ملف مش مفهوم');
  // لازم يكون فيه على الأقل جدول واحد معروف — يمنع إن حد يرفع أي ملف JSON بالغلط
  if (!BACKUP_TABLES.some((t) => Array.isArray(data[t]))) throw new Error('الملف ده مش نسخة احتياطية');
  for (const t of BACKUP_TABLES) if (Array.isArray(data[t])) saveLocal(KEYS[t], data[t]);
...
// عدد السجلات جوه نسخة احتياطية — بيتعرض للمستخدم قبل/بعد الاسترجاع
export function countBackup(data) {
  const c = {};
  for (const t of BACKUP_TABLES) if (Array.isArray(data[t])) c[t] = data[t].length;
  return c;
}

// آخر مرة اتنزّلت فيها نسخة احتياطية على الجهاز (مش السحابة)
const LAST_BACKUP_KEY = 'saqqa_last_local_backup';

export function markBackupDone() {
  try { localStorage.setItem(LAST_BACKUP_KEY, new Date().toISOString()); } catch {}
}

// بيرجع { at, days } — days = null يعني عمره ما اتعملت نسخة على الجهاز ده
export function backupInfo() {
  if (typeof window === 'undefined') return { at: null, days: null };
  const at = localStorage.getItem(LAST_BACKUP_KEY);
  if (!at) return { at: null, days: null };
  const days = Math.floor((Date.now() - new Date(at).getTime()) / 86400000);
  return { at, days: Number.isFinite(days) ? days : null };
}
...
// ترحيل أنواع العملاء للنموذج الجديد (نوعين بس): قطاعي→نقدي · جملة/موزع→تاجر جملة
// بيشتغل مرة واحدة على كل جهاز، وبيتزامن للسحابة
export function migrateCustomerTypes() {
  if (typeof window === 'undefined') return 0;
  if (localStorage.getItem('saqqa_custtype_v') === '1') return 0;
  const map = { 'قطاعي': 'نقدي', 'عادي': 'نقدي', 'جملة': 'تاجر جملة', 'موزع': 'تاجر جملة' };
  const list = loadLocal(KEYS.customers, []);
  let changed = 0;
  for (const c of list) {
    const nt = map[c.priceType];
    if (nt && nt !== c.priceType) {
      c.priceType = nt;
      c.updated_at = todayISO();
      queuePush('customers', 'upsert', c);
      changed++;
    }
  }
  if (changed) {
    saveLocal(KEYS.customers, list);
    logAudit('ترحيل أنواع العملاء', `تحويل ${changed} عميل للنموذج الجديد (نقدي/تاجر جملة)`);
  }
  localStorage.setItem('saqqa_custtype_v', '1');
  return changed;
}
