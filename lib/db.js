...
// تصحيح أسماء الأصناف زي ملف الشركة الأصلي: شيل المسافات حوالين الرموز
// "ط / كاس" → "ط/كاس" — "6 * 1" → "6*1" — "7001 - 1" → "7001-1"
...
export async function fixSlashNames() {
  ...
  if (localStorage.getItem('saqqa_slash_fix') === '2') return 0;
  const all = loadLocal(KEYS.products, []);
  ...
  // 2) تصحيح الأسماء زي ملف الشركة
  let changed = 0;
  for (const p of keep) {
    const fixed = cleanProductName(p.name);
    if (fixed !== p.name) {
      p.name = fixed;
  ...
  if (changed || removedIds.length) {
    logAudit('تصحيح أسماء', `تصحيح ${changed} اسم وحذف ${removedIds.length} صنف مكرر`);
  }
  localStorage.setItem('saqqa_slash_fix', '2');
  return changed;
}
