...
export async function fetchProductsCloud() {
  ...
  try {
    ...
    return dedupByCode((data || []).map((r) => ({ ...r.data, id: r.id })));
  } catch {
    return null;
  }
}
...
// تصحيح أسماء الأصناف زي ملف الشركة الأصلي: "ط / كاس" → "ط/كاس" (شيل المسافات حوالين الشرطة)
// بيشتغل مرة واحدة على كل جهاز — والتعديل بيتزامن للسحابة وباقي الأجهزة
export function fixSlashNames() {
  if (typeof window === 'undefined') return 0;
  if (localStorage.getItem('saqqa_slash_fix') === '1') return 0;
  const list = loadLocal(KEYS.products, []);
  let changed = 0;
  for (const p of list) {
    const fixed = String(p.name || '').replace(/\s*\/\s*/g, '/');
    if (fixed !== p.name) {
      p.name = fixed;
  ...
  if (changed) {
    saveLocal(KEYS.products, list);
    logAudit('تصحيح أسماء', `تصحيح مسافات الشرطة في ${changed} صنف`);
  }
  localStorage.setItem('saqqa_slash_fix', '1');
  return changed;
}
