...
// نسخة الإعدادات اللي بتترفع للسحابة — من غير كلمات السر
...
// دمج إعدادات السحابة مع الحفاظ على كلمات السر المحلية (متجيش من السحابة أبداً)
function mergeCloudSettings(cloud) {
  const local = loadLocal(KEYS.settings, null) || {};
  const kept = {};
  for (const k of SECRET_KEYS) if (local[k] !== undefined) kept[k] = local[k];
  return { ...DEFAULT_SETTINGS, ...cloud, ...kept };
}
